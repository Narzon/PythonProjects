...
#Import random for shuffling, and initialize a dictionary (valdict) for card pips 
import random
valdict = {}

#Create a list of all possible suits and pips for each of the 52 playing cards. In this case, T=10 as a pip
suitlist = ["C", "D", "H", "S"]
piplist = ["A", "2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K"]
...
#Create a Card, given a suit and pip
class Card:
    def __init__(self, suit, pip):
        self.suit = suit
        self.pip = pip

#Give each Card a pip, with T/J/Q/K being equal to 10, and A initially being equal to 11. Add each card as a key to the dictionary (valdict), along with its int pip
        if  pip == "J" or pip == "Q" or pip == "K" or pip =="T":
            pip = 10
        elif pip == "A":
            pip = 11
        else:
            pip = int(pip)
        valdict["%s%s" % (self.pip, self.suit)] = pip
        self.actualpip = pip
...
#Given the number of aces reduced to 1 (redaces), return the total pip of all cards in a player's hand
...
#Create variables redaces (redacesD for dealer) to keep track of many aces in each player's hand is being reduced to a pip of 1
    redaces = 0
    redacesD = 0
...
#Create an automatic loop for the dealer. If the opponent has a higher pip hand, keep drawing cards. If the dealer busts, the opponent wins. If the dealer gets an equal or higher pip to the opponent, the dealer wins
